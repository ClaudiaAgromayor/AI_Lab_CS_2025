...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        if start_position in self.corners:
            visited_corners = frozenset([start_position])
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if new_position in self.corners:
                    new_visited_corners = visited_corners | frozenset([new_position])
                    #autre façon: new_visited_corners = visited_corners.union(frozenset([new_position]))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in unvisited:
        x_distance = abs(position[0] - corner[0])  # Horizontal distance
        y_distance = abs(position[1] - corner[1])  # Vertical distance
        total_distance = x_distance + y_distance   # Add them together
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    distances_to_position = [abs(position[0] - food[0]) + abs(position[1] - food[1]) for food in foodList]
    nearest_distance = min(distances_to_position)
    
    # Calculate MST approximation
    food_dists = []
    ...
    food_dists.sort()
    mst_cost = sum(food_dists[:len(foodList)-1]) if food_dists else 0
    """
    len(foodList) = 4
    len(foodList) - 1 = 3

    food_dists = [4, 4, 4, 4, 8, 8]  # All distances between foods (sorted)
    food_dists[:3] = [4, 4, 4]   
    """
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        x,y = state
        "*** YOUR CODE HERE ***"
        return self.food[x][y] # Goal is reached when Pacman is at a position with food
